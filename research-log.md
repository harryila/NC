...
## 2026-05-30 — Phase-2 #3 built (trajectory driver + H3 snapshot loop + hardened gate)
Verify→draft→adversarial-review workflow. Verified the exact Avalanche per-experience key `Top1_Acc_Exp/eval_phase/test_stream/Task000/Exp00j` and that `get_last_metrics()` is final-only → must snapshot per-eval to build the accuracy matrix A[i,j]. Delivered:
- `avalanche_backbone.run_split_cifar100` now records the full **accuracy matrix → learning_acc (A[k,k]) / avg_forgetting / bwt**, and (flag `snapshot_h3`, on for R5/R6 arms) snapshots H3 features + oscillator state per task via a **fixed class-balanced probe loader**; returns a rich dict.
- `run_matrix.py` unpacks that dict into the results json (flat metrics + learning_acc/acc_matrix/h3 at top level) — the load-bearing glue the review caught both drafts omitting.
- `analyze_hardened.py` (sibling to the simple `analyze.py`): **plasticity guard** (one-sided non-inferiority TOST on learning-acc — a forgetting win is INVALIDATED if R6 underfit), **Holm** over the 3 R5 brackets, **intersection-union** GREENLIGHT over the now-**5-way** conjunction {forgetting effect, 20×5 replication sign, plasticity, H3 DiD, **A5-competitiveness**}, and **positive-control-guarded PIVOT-A/PIVOT-B** (can't declare a null until #4 exists). `--demo` GREENLIGHTs/INVALIDATEs/INCONCLUSIVEs correctly.
